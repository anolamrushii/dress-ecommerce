    ...
    return (
      <div className="rounded border border-dashed border-gold-light/60 bg-ivory/40 px-6 py-12 text-center">
        <p className="font-body text-sm text-muted-foreground">
          No dresses yet. Add your first one below.
        </p>
      </div>
    );
  }
      ...
      {dresses.map((dress) => (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-heading text-lg leading-tight text-charcoal">{dress.name}</p>
            <p className="truncate font-body text-xs text-muted-foreground">
              {dress.slug} &middot; {dress.price ? `$${dress.price}` : "Inquire"}
            </p>
            <div className="mt-2 flex flex-wrap gap-1.5">
              <Badge tone={dress.is_published ? "gold" : "neutral"}>
                {dress.is_published ? "Published" : "Draft"}
              </Badge>
              {dress.is_featured && <Badge tone="gold">Featured</Badge>}
              <Badge tone="neutral">
                {dress.images.length} photo{dress.images.length === 1 ? "" : "s"}
              </Badge>
            </div>
          </div>

          <div className="flex shrink-0 flex-col gap-1">
            <button onClick={() => onEdit(dress)} aria-label="Edit" className={ICON_BUTTON}>
              <svg
